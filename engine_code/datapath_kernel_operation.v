...
module sobel_datapath #(
    ...
    abs_ip #(.WIDTH(12)) u_abs_gx    (.a(sum_x), .abs(gx));
    abs_ip #(.WIDTH(12)) u_abs_gy    (.a(sum_y), .abs(gy));
    ...
    wire                        com_threshold      ;
    wire    [clipped_w - 1:0]   clipped_rsult   ;
    // Determine if the mag meets the threshold requiremnet. In the completed software poart, threshold = 60.
    com_ip  #(.WIDTH(12)) u_com_threshold   (.a(mag), .b({4'd0, threshold}), .com(com_threshold));
    // Get the last 8 bits
    clip_ip #(.WIDTH(12)) u_clip_8bits      (.a(mag), .clip(clipped_result)                     );
